...
Datetimecombo.prototype.html = function(callback) {
	...
	for(i=h1; i <= h2; i++) {
	    val = i < 10 ? "0" + i : i;
	    text += '<option value="' + val + '" ' + (i == this.hrs ? "SELECTED style='background-color:#f00;color:#fff;'" : "") +  '>' + val + '</option>';
	}
	...
	if(this.hasMeridiem) {
		...
		text += '\n<option value="' + (this.pm ? "am" : "AM") + '" ' + (/am/i.test(this.meridiem) ? "SELECTED style='background-color:#f00;color:#fff;'" : "") + '>' + (this.pm ? "am" : "AM") + '</option>';
		text += '\n<option value="' + (this.pm ? "pm" : "PM") + '" ' + (/pm/i.test(this.meridiem) ? "SELECTED style='background-color:#f00;color:#fff;'" : "") + '>' + (this.pm ? "pm" : "PM") + '</option>';
		text += '\n</select>';
	}
