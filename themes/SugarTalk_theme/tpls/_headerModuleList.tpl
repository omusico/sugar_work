...
{*        <span class="currentTab">*}
        {if $group != "Все"}
            <a href="#" id="grouptab_{$smarty.foreach.groupList.index}">
                <img class="img_tab" src = "./themes/SugarTalk_theme/images/mainmenu_{$group}.png" />
                {$group}
            </a>
        {else}
            <a href="#" id="grouptab_{$smarty.foreach.groupList.index}">
                <span>{$group}<br/> разделы</span>
                <img class="img_tab" src = "./themes/SugarTalk_theme/images/mainmenu_{$group}.png" />
            </a>
        {/if}
{*        </span>*}
{*         <span class="currentTabRight">&nbsp;</span>*}
        {assign var="groupSelected" value=true}
      </li>
    {elseif $smarty.foreach.groupList.index <= 9}
...
{* SEARCH FORM *}
{if $AUTHENTICATED}
<div id="search" class="non">
    <form name='UnifiedSearch' onsubmit='return SUGAR.unifiedSearchAdvanced.checkUsaAdvanced()'>
        <input type="hidden" name="action" value="UnifiedSearch">
        <input type="hidden" name="module" value="Home">
        <input type="hidden" name="search_form" value="false">
        <input type="hidden" name="advanced" value="false">
        <input type="text" name="query_string" placeholder="Поиск..." id="query_string" size="20" value="{$SEARCH}">&nbsp;
        <img id="unified_search_advanced_img" src="./themes/SugarTalk_theme/images/icon_for_search_form.png" border="0" alt="{$APP.LBL_SEARCH}">&nbsp;
     </form>
