...
*}
<!--end body panes-->
{assign var = COPYRIGHT     value = "&copy 2008-2014 SugarTalk"}
        </td></tr></table>
 <div class="clear"></div>
 {if $AUTHENTICATED}
{*    <div id="bottomLinks">
...
<div  id="footer">
    <div id="responseTime">
    {$STATISTICS}
    <img src="./themes/SugarTalk_theme/images/logo_footer.png" />
    </div>
    <div id="copyright">
        <b>CRM for realty</b><br/>
        &copy 2008-{php}echo date('Y');{/php} SugarTalk
    </div>
</div>
<script>
{literal}
...
$(document).ready(function(){
   {/literal}
   {if $smarty.request.module != "Campaigns"}
   {literal}
   setTimeout(function(){
       //alert(document.getElementById('main').clientHeight)
       // $('#main').css('min-height', "100%");

        $('#main #content').after('<div id="footer_before"></div>');
        $('#footer_before').css('height', document.getElementById('footer').clientHeight);
        $('#footer').css('margin-top', -document.getElementById('footer').clientHeight);

        var detail_offset_height = $("#content .detail.view.detail508 table tr td[scope='col']").outerWidth();
        // console.log(detail_offset_height);
        $("div#content-gallery div#gallery > div").css("right", detail_offset_height);
   }, 900);
   {/literal}
   {/if}
   {literal}
