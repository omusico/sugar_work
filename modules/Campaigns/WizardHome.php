...
function create_marketing_summary  ($focus){
    ...
    $mrkt_tbl .= "<tr  class='listViewHRS1'><td scope='col' width='15%'><b>".$mod_strings['LBL_MRKT_NAME']."</b></td>";
	# tracy
	if ($focus->campaign_type!='SMS') {
...
# tracy
if ($focus->campaign_type!='SMS') $mrkt_tbl  .= "<td scope='row' width='25%'>".$mrkt_focus->from_name."</td>";
...
function create_wiz_menu_items($type,$mrkt_string,$camp_url,$summ_url){
    ...
    if(isset($steps)  && !empty($steps)){
        ...
        foreach($steps as $name=>$step){
           	if (!($type == "sms" && ($name == "Trackers" || $name == "Subscriptions"))) {
