...
public function getDuplicateQuery($focus, $prefix='')
{
    ...
    if(!empty($_POST[$prefix.'record'])) {
        $query .= "contacts.id != '". $_POST[$prefix.'record'] ."'";
    }

    //Comment, because there is no need in this logic
...
//	}
    ...
    $query .= " AND ( ";
    //Iluxovi4
    if(isset($_POST[$prefix.'phone_home']) && strlen($_POST[$prefix.'phone_home']) != 0){
        $query .= " phone_home = '". $_POST[$prefix.'phone_home'] . "' ";
    }
    if(isset($_POST[$prefix.'phone_work']) && strlen($_POST[$prefix.'phone_work']) != 0) {
        $query .= " OR phone_work = '". $_POST[$prefix.'phone_work'] ."'";
    }
    if(isset($_POST[$prefix.'phone_mobile']) && strlen($_POST[$prefix.'phone_mobile']) != 0) {
        $query .= " OR phone_mobile = '". $_POST[$prefix.'phone_mobile'] ."'";
    }
    $query .= " )";
...
function handleRedirect($return_id){
    ...
    //$return_action = "EditView"; // if we "Cancel", we go back to the edit view. {Iluxovi4}
