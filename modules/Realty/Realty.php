...
require_once('modules/Realty/Realty_sugar.php');
class Realty extends Realty_sugar {
    var $importable = true;
...
//        }
    ...
    public function save($check_notify=false)
    {
        ...
        {
            $res = $db->query("SELECT address_country,address_city,address_street,address_house FROM realty WHERE id = '{$this->id}' ");
            ...
            $db->query("UPDATE realty SET latitude = '{$lat}', longitude = '{$lng}' WHERE id = '{$this->id}' ");
        }
...
//        $where = str_replace('\"', '"', $where);
        ...
        global $db;
        $sql = 'SELECT realty.latitude, realty.longitude, realty.name, realty.id '.$ret_array['from'].$ret_array['where'];
        $res = $db->query($sql);
