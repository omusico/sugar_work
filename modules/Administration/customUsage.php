<?php
	...
	echo "<script type=\"text/javascript\" language=\"JavaScript\" src=\"modules/Administration/sugartalk_customUsage/javascript/jquery.js\"></script>";
	echo "<script type=\"text/javascript\" language=\"JavaScript\" src=\"modules/Administration/sugartalk_customUsage/javascript/customUsage.js\"></script>";
	echo "<link rel=\"stylesheet\" type=\"text/css\" href=\"modules/Administration/sugartalk_customUsage/style/customPhone.css\" />";
	...
	$module_name_lower = "sugartalk_sms";
	...
	if(isset($_GET["fields"]) && !empty($_GET["fields"])) {
		...
		foreach($_GET["fields"] as $key) {
			...
			if(array_key_exists($key ,$modules)){ 
				...
				//write vardef files for sugartalk_Usage.
				$object= $GLOBALS['beanList'][$key];
				if ($object == 'aCase') $object = 'Case';
				$the_string = "<?php\n" . "// created: " . date('Y-m-d H:i:s') . "\n";
		...
		echo "<div class='moduleTitle'><h2><a href='index.php?module=Administration&action=index'>Administration</a><span class='pointer'>&raquo;</span>Связь модулей с модулем SMS</h2></div>";	// just a space

		$modules = $GLOBALS['moduleList']; 	
		asort($modules);
		echo "<div id='field_panels'></div>";
		echo "<div style='margin-bottom:5px;'><input type='button' id='save' class='button' value='Сохранить' disabled='disabled'></div>";
		echo "<table id='mod_tbl' border='0' cellspacing='0' cellpadding='0' class='edit view'  width='100%' >";
		echo "<tr><th style='text-align:left;'>Переместите модуль в правую панель, чтобы создать связь с SMS</th></tr>";
 		echo "<tr><td>"; 
		...
		foreach($modules as $mod) {
			...
			if(!array_key_exists($mod, $GLOBALS["app_list_strings"][$module_name_lower."_module_selected_list"])){
				echo "<div class=\"item\" id=\"{$mod}\" onclick=\"select(this);\" title=\"Нажмите, чтобы выбрать или отменить\">";
		...
		echo "<div class='item' id=\"-BLANK-\" onclick='select(this);' title='Нажмите, чтобы выбрать или отменить' style=\"display:none;\">-BLANK-</div>";
		if(!empty($GLOBALS["app_list_strings"][$module_name_lower."_module_selected_list"])){
			foreach($GLOBALS["app_list_strings"][$module_name_lower."_module_selected_list"] as $k=>$v){
				if($k!='-BLANK-'){
					echo "<div class='item' id=\"{$k}\" onclick='select(this);' title='Нажмите, чтобы выбрать или отменить'>" . $v . "</div>";
				}
			}
		} 
