...
*}
	...
	{if $fields.reminder_time}            	
                ...
                {assign var="SMS_REMINDER_TIME_OPTIONS" value=$fields.reminder_time.options}
            	...
            	{/if}
                {if !$fields.sms_reminder_checked.value}
                    {assign var="SMS_REMINDER_TIME" value=-1}
                    {else}
                    {assign var="SMS_REMINDER_TIME" value=$fields.sms_reminder_time.value}
                {/if}
	{/if}
	
	{assign var="REMINDER_TIME_DISPLAY" value="none"}
	{assign var="EMAIL_REMINDER_TIME_DISPLAY" value="none"}
    {assign var="SMS_REMINDER_TIME_DISPLAY" value="none"}
	{if $REMINDER_TIME != -1}
	...
	{/if}
        ...
        {/if}
        {if $SMS_REMINDER_TIME != -1}
            {assign var="SMS_REMINDER_CHECKED" value="checked"}
            {assign var="SMS_REMINDER_TIME_DISPLAY" value="inline"}
        {/if}
	...
	{if $view == "EditView" || $view == "QuickCreate" || $view == "QuickEdit" || $view == "wirelessedit"}
        ...
        <div>
            ...
            <input name="sms_reminder_checked" type="hidden" value="0">
            <input name="sms_reminder_checked" id="sms_reminder_checked" onclick="toggleReminder(this,'sms_reminder');" type="checkbox" class="checkbox" value="1" {$SMS_REMINDER_CHECKED}>
            <div style="display: inline-block; width: 111px;">{$MOD.LBL_REMINDER_SMS_ALL_INVITEES}</div>
            <div id="sms_reminder_list" style="display: {$SMS_REMINDER_TIME_DISPLAY}">
                <select tabindex="{$REMINDER_TABINDEX}" name="sms_reminder_time">
                    {html_options options=$SMS_REMINDER_TIME_OPTIONS selected=$SMS_REMINDER_TIME}
                </select>
            </div>
        </div>
	...
	{else}
        ...
        <div>
            <input type="checkbox" disabled  {$SMS_REMINDER_CHECKED}>
            {$MOD.LBL_REMINDER_SMS_ALL_INVITEES}
            {if $SMS_REMINDER_TIME != -1}
                {$SMS_REMINDER_TIME_OPTIONS[$SMS_REMINDER_TIME]}
            {/if}
        </div>
	{/if}	
