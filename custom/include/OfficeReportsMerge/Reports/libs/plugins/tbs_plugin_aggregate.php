...
Add operation to sum function (by Edward Malik)
********************************************************
*/
...
class clsTbsAggregate {
	...
	function f_Ope_Sum(&$Ope,&$CurrRec) {
        $additional_operation = array('*','+','-','/');
        foreach ($additional_operation as $operation)
        {
            if (strpos($Ope->OrigCol, $operation) !== FALSE AND strpos($Ope->OrigCol, $operation) !== strlen($Ope->OrigCol) - 1)
            {
                //$value = explode($operation, $Ope->OrigCol);
                $Ope->Value += $this->calculate_string($Ope->OrigCol, $CurrRec);
                return;
            }
        }
    ...
    function calculate_string( $mathString, $values )
    {
        $mathString = trim($mathString);     // trim white spaces
        foreach ($values as $key => $val)
        {
            $mathString = str_replace($key, $val, $mathString);
        }
