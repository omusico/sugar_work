...
<script type='text/javascript' src='{sugar_getjspath file='custom/modules/Realty/tpls//popup_hook.js'}'></script>
				...
				{foreach from=$displayColumns key=col item=params}
					<td scope='row' align='{$params.align|default:'left'}' valign=top class='{$_rowColor}S1' bgcolor='{$_bgColor}'>
						{if $params.link && !$params.customCode}
							...
							<{$pageData.tag.$id[$params.ACLTag]|default:$pageData.tag.$id.MAIN} href='javascript:void(0)' onclick="check_realty('{$rowData[$params.id]|default:$rowData.ID}','{$smarty.request.op_id}');">{$rowData.$col}</{$pageData.tag.$id[$params.ACLTag]|default:$pageData.tag.$id.MAIN}>
