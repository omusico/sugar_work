...
class SugarWebServiceImplv3_1 extends SugarWebServiceImplv3 {
    ...
    */
     ...
     * @param Array $images -- SugarTalk core customization for SugarMobile
     * @return Array    'id' -- the ID of the bean that was written to (-1 on error)
     * @exception 'SoapFault' -- The SOAP error, if any
     */
    function set_entry($session,$module_name, $name_value_list, $track_view = FALSE, $images = NULL ){
        ...
        $GLOBALS['log']->info('End: SugarWebServiceImpl->set_entry');
        $GLOBALS['log']->fatal('Log Test SugarTalk Mobile');
        file_put_contents( "log.txt", var_export($images, 1) );
        //SugarTalk customization for Mobile === addImages ===
        if($module_name == "Realty"  
                && !empty($images) ) {
            $dir = "upload/gallery_images/{$seed->id}/";
            mkdir($dir);
            $separator = "^|^";
            $i = 0;
            foreach($images as $image) {
                $file_name = create_guid().".jpg";
                $data = $image;
                $data = base64_decode($data);
                $img = imagecreatefromstring($data);
                $temp_arr[] = imagejpeg($img, $dir.$file_name);
                if($i==0) $gallery="^,^{$file_name}^,^^,^^,^^,^^,^^,^off^,^off^,^off^";
                elseif($i>0) $gallery.= $separator."^,^{$file_name}^,^^,^^,^^,^^,^^,^off^,^off^,^off^";
                $i++;
            }
            $db = DBManagerFactory::getInstance();
            $r = $db->query("SELECT id_c FROM realty_cstm WHERE id_c = '{$seed->id}' ");
            if( $row = $db->fetchByAssoc($r) ) {
                $db->query("UPDATE realty_cstm SET galleria_c = '{$gallery}' WHERE id_c = '{$seed->id}' ");
            } else 
                $db->query("INSERT INTO realty_cstm (id_c, realty_cstm.galleria_c) VALUES ('{$seed->id}', '{$gallery}') ");
            return array('id'=>$seed->id, 'entry_list' => $return_entry_list, 'images'=>$temp_arr);
        }
        //=====================================================  
