...
class EditView
{
    ...
    function display($showTitle = true, $ajaxSave = false)
    {
        ...
        /**
         * bagfix
         * фикс роли, т.к. она не правильно работала
         * из-за функции create_guid() в конструкторе класса Realty в файле Realty.php
         */
        if($this->focus->object_name == "Realty" || $this->focus->object_name == "Buildings")
        {
            if(empty($this->fieldDefs['id']['value']) || (isset($this->returnId) && $this->isDuplicate) )
            {
                $new_with_id = create_guid();
                $this->focus->new_with_id = 1;
                $this->th->ss->assign('new_with_id', $new_with_id);
            }
        }
    ...
    public function showTitle($showTitle = false)
    {
        ...
        if ($showTitle)
        {
			$module_path_link = "<div style='float:left;'><h2><a href='index.php?module={$this->module}'>
			{$GLOBALS['app_list_strings']['moduleList'][$this->module]}</a> /&nbsp;</h2></div>";
            return $module_path_link.$this->viewObject->getModuleTitle();
        }
