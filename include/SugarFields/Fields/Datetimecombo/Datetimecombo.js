...
this.meridiem=this.hasMeridiem?trim(this.datetime.substring(16)):'';this.datetime=this.datetime.substr(0,10);this.showCheckbox=showCheckbox;this.checked=parseInt(checked);YAHOO.util.Selector.query('input#'+this.fieldname+'_date')[0].value=this.datetime;if(this.mins>0&&this.mins<55){for(var i=0;i<=50;i=i+5){if(this.mins>i&&this.mins<i+5){this.mins=i+5;}}}else if(this.mins>55){this.hrs+=1;this.mins=0;if(this.hasMeridiem&&this.hrs==12){if(this.meridiem=="pm"||this.meridiem=="am"){if(this.meridiem=="pm"){this.meridiem="am";}else{this.meridiem="pm";}}else{if(this.meridiem=="PM"){this.meridiem="AM";}else{this.meridiem="PM";}}}
...
for(i=h1;i<=h2;i++){val=i<10?"0"+i:i;text+='<option value="'+val+'" '+(i==this.hrs?"SELECTED style='background-color:#f00;color:#fff;'":"")+'>'+val+'</option>';}
...
for(i=0;i<=55;i=i+5)
{val=i<10?"0"+i:i;text+='\n<option value="'+val+'" '+(this.mins==i?"SELECTED style='background-color:#f00;color:#fff;'":"")+'>'+val+'</option>';}
...
text+='\n<option value="'+(this.pm?"am":"AM")+'" '+(/am/i.test(this.meridiem)?"SELECTED style='background-color:#f00;color:#fff;'":"")+'>'+(this.pm?"am":"AM")+'</option>';text+='\n<option value="'+(this.pm?"pm":"PM")+'" '+(/pm/i.test(this.meridiem)?"SELECTED style='background-color:#f00;color:#fff;'":"")+'>'+(this.pm?"pm":"PM")+'</option>';text+='\n</select>';}
