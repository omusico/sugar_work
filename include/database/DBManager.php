...
protected function checkQuery($sql, $object_name = false)
{
	...
	public function repairTableParams($tablename, $fielddefs,  $indices, $execute = true, $engine = null)
	{
		...
		foreach ($fielddefs as $name => $value) {
			...
			if ( !isset($compareFieldDefs[$name]) ) {
				...
				$take_action = true;
			} elseif ( !$this->compareVarDefs($compareFieldDefs[$name],$value, false,$name)) {
     ...
     */
	public function compareVarDefs($fielddef1, $fielddef2, $ignoreName = false, $name='')
	{
		foreach ( $fielddef1 as $key => $value ) {
			...
			if($key=='required' && $name=='code_inc'){ // Kolerts: very dirty hack))
				continue;
			}	
			/* echo $name.'<hr/>';
			if($name=='code_inc'){
				echo '<br/>fucken '.$key.'<br/><br/>';
			} */
			return false;
		}
